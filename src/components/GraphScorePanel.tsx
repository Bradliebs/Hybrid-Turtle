        ...
        <div className="flex-1 h-2 bg-navy-800/60 rounded-full overflow-hidden">
          <div
            className={cn(
              ...
              data.gnnScore > 0.6 ? 'bg-emerald-500/70' :
              data.gnnScore < 0.4 ? 'bg-red-500/70' : 'bg-amber-500/70'
            )}
            style={{ width: `${data.gnnScore * 100}%` }}
          />
        </div>
        <span className="text-[10px] font-mono text-muted-foreground w-8 text-right">
          {((data.gnnScore ?? 0.5) * 100).toFixed(0)}%
        </span>
      </div>
      ...
      {!data.modelTrained && (
        <div className="flex items-center gap-1.5 mt-1">
          ...
          <span className="text-[10px] text-muted-foreground/50">
            Scores are baseline until ≥150 outcomes available
          </span>
        </div>
      )}
    </div>
